...
public class FrequentSetIterator implements Iterator<FrequentSet<Long>> {
    ...
    public FrequentSetIterator(TransportClient transportClient, String indexName,
            Integer level) {
    ...
    private void provideCandidates() {
        ...
        collection.stream()
                .forEach(searchHit -> {
                    ...
                    Double support = (Double) searchHit.field("support").
                            getValue();
                    ...
                    frequentSet.setItems(transactionValues.stream().collect(
                            Collectors.toSet()));
