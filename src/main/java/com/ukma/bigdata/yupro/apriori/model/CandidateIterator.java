...
public class CandidateIterator implements Iterator<Set<Long>> {
    ...
    public CandidateIterator(TransportClient transportClient,
            Queue<Set<Long>> candidates, String indexName, Integer level) {
    ...
    private void provideCandidates() {
        SearchResponse searchResponse = transportClient.prepareSearch(String.
                format("%s",
                        this.indexName)).setScroll(new TimeValue(
                        60000)).
                setSize(100).get();
        ...
        collection.stream()
                .forEach(s -> {
                    Set<Long> candidateSet = (Set<Long>) (Object) s.getFields().
                            get("transactionValues").getValue();
                    this.candidates.add(candidateSet);
                });
    }
